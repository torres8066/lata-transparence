...
export default function EditReport() {
  ...
  const getReport = useAppStore(state => state.getReport);

  const [formData, setFormData] = useState<Partial<ClientReport> | null>(null);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updateReport(id, formData as ClientReport);
    navigate('/');
  };
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        setFormData(prev => prev ? ({ ...prev, lienPhotoValise: reader.result as string }) : null);
      };
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 pb-20">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-2xl font-bold text-neutral-100 tracking-tight">Modifier Rapport Client</h2>
          <p className="text-neutral-500 mt-1">Mettez à jour les informations techniques de l'atelier.</p>
        </div>
      </div>
            ...
            <div>
              ...
              {formData.lienPhotoValise && (
                 <div className="mt-3 relative inline-block rounded-xl overflow-hidden border border-neutral-800 p-1">
                   ...
                   <button type="button" onClick={() => setFormData(p => p ? ({...p, lienPhotoValise: ''}) : null)} className="absolute top-1 right-1 bg-red-500/80 text-white p-1 rounded-full hover:bg-red-500 transition-colors"><X size={14}/></button>
                 </div>
              )}
            </div>
          </div>
